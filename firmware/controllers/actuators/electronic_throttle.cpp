...
struct EtbImpl final : public TBase {
	...
	if (TBase::m_isAutocal) {
	  ...
	  efiPrintf("Opening!");
		...
		float secondaryMax = primaryMax;
		if (!engineConfiguration->allowIdenticalPps)
		{
			secondaryMax = Sensor::getRaw(functionToTpsSensorSecondary(myFunction));
		}
		...
		float secondaryMin = primaryMin;
		if (!engineConfiguration->allowIdenticalPps)
		{
			secondaryMin = Sensor::getRaw(functionToTpsSensorSecondary(myFunction));
		}
